...
export default function CrawlerInterface() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200 flex items-center justify-center font-mono p-4">
      <div className="w-full max-w-2xl p-8 bg-slate-800 rounded-xl shadow-2xl border border-slate-700">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-slate-900 border border-slate-700 rounded px-4 py-2 focus:outline-none focus:border-green-500 transition-colors"
          />
          <button
            onClick={() => startJob(url)}
            disabled={status === 'crawling' || !url}
            className="bg-green-600 hover:bg-green-500 text-white px-6 py-2 rounded font-bold disabled:opacity-50 transition-all"
          >
        ...
        <div className="mt-6 min-h-[50px]">
          {status === 'success' && (
            <div className="space-y-4">
              ...
              {showData && data && (
                <div className="p-4 bg-slate-900 rounded border border-slate-700 overflow-auto max-h-96 text-xs">
          ...
          {status === 'timeout' && (
            <div className="p-3 bg-yellow-900/30 border border-yellow-500/30 rounded text-yellow-300 text-sm">
